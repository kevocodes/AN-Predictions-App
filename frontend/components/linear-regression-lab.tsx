...
export default function LinearRegressionLab() {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      const response = await fetch("/api/predict/regression/batch", {
        method: "POST",
        body: formData,
      });
  ...
  const downloadTemplate = async () => {
    try {
      const response = await fetch("/api/templates/linear");
      if (!response.ok) {
      ...
      a.href = url;
      a.download = "plantilla_regresion_lineal.xlsx";
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Evalúa el modelo de regresión lineal para predecir el consumo
          energético en un edificio de bajo consumo, ya sea ingresando datos
          individuales o cargando un archivo Excel con múltiples registros.
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <TabsContent value="individual">
            ...
            {prediction !== null && (
              <div className="mt-6 p-4 bg-muted rounded-md">
                ...
                <p className="text-2xl font-bold">{prediction} Wh</p>
              </div>
            )}
          </TabsContent>
